...
import {SectionList, Text, View, TouchableOpacity, Button as NativeButton, Dimensions, ProgressViewIOS, Image} from "react-native"
...
import {changeExerciseProp, addExercise, clearExerciseProgress, toggleExerciseClick, resetClickedExercises} from "../actions/exerciseActions";
...
class ExerciseList extends React.Component
{
  ...
  renderItem(all){
    ...
    return <ListItem navigation={this.props.navigation} item={item} clicked={clicked} weight={weight} reps={reps} sets={sets} notes={notes} regionKey={section.regionKey} onPress={() => this.handleClick(all)}/>
  }
  ...
  resetState(){
    ...
    this.setState({stopwatchStart: false, stopwatchReset: true},() => this.setState({stopwatchReset: false, stopwatchStart: true}))
  }

  renderFooter(){
    return (
      <View style={{backgroundColor: "black" , justifyContent: "space-around", alignItems: "center", width: "100%", height: 50, flexDirection: "row", borderTopWidth: 0.5, borderColor: "gray"}}>
        <View style={{flex: 1}}>
        ...
        <View style={{flex: 1}}>
          <Button text={"Finish"} color={"white"} fontWeight={"bold"} onPress={() => this.props.navigation.navigate("BodyRegionSelection", {title: this.props.lrc.BodyRegionSelection.title})}/>
        </View>
      </View>
    )
  }
  ...
  render(){
    ...
    return (
      <View style={{height: "100%", backgroundColor: "white"}}>
        ...
        />
        <ModalAddExercise visible={this.state.modalVisible} onCloseModal={this.onCloseModal.bind(this)} animationType={"fade"} transparent={true}/>
        {this.renderFooter()}
      </View>
    )
...
export default connect(mapStateToProps,{changeEditExercise, addExercise, changeExerciseProp, clearExerciseProgress, toggleExerciseClick, resetClickedExercises})(ExerciseList)
